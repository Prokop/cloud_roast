...
from cloudcafe.identity.v3.common.tokens.models import responses, requests
...
class TokensClient(BaseIdentityAPIClient):
    ...
    def authenticate(self, token_id=None, user_id=None, username=None,
        ...
        if token_id is not None:
            kwargs["identity"] = requests.Identity(
    ...
    def check_token(self, token, nocatalog=None, requestslib_kwargs=None):
        """
        @summary: Check a token
        @param token: Token to be checked
        @type token: String
        @return: Check token information
        @rtype: Catalog List
        """
        ...
        url = "{url}/auth/tokens".format(url=self.url)
        params = {"nocatalog": nocatalog}
        return self.head(url, params=params,
                        headers={'x-subject-token': token},
                        response_entity_type=responses.AuthResponse,
                        requestslib_kwargs=requestslib_kwargs)