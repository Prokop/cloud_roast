...
class NetworkingFixture(BaseTestFixture):
    ...
    def create_test_network(self, expected_network, set_up=True):
        ...
        if (resp.response.status_code == NeutronResponseCodes.CREATE_NETWORK
            ...
            expected_network.name = 'cloudroast-' + expected_network.name
...
class NetworkingAPIFixture(NetworkingFixture):
    ...
    def setUpClass(cls):
        ...
        cls.subnet_data = dict(
            name='test_subnet',
            tenant_id=cls.user.tenant_id,
            enable_dhcp=True, dns_nameservers=[], gateway_ip=None,
            host_routes=[])
...
class NetworkingSecurityGroupsFixture(NetworkingAPIFixture):
    ...
    def create_test_secgroup(self, expected_secgroup=None, delete=True):
        ...
        expected_secgroup_copy = copy(expected_secgroup)
        ...
        expected_secgroup_copy.name = 'cloudroast-'+expected_secgroup_copy.name
        ...
        self.assertSecurityGroupResponse(expected_secgroup_copy, secgroup,
                                         check_exact_name=False)
        return secgroup
    ...
    def create_test_secrule(self, expected_secrule, delete=True):
        ...
        expected_secrule_copy = copy(expected_secrule)
        request_kwargs = dict(
        ...
        if expected_secrule.protocol:
            ...
            expected_secrule_copy.protocol = 'any'

        if not expected_secrule.port_range_min:
            expected_secrule_copy.port_range_min = 0

        if not expected_secrule.port_range_max:
            expected_secrule_copy.port_range_max = 65535
        ...
        # Check the Security Group response
        self.assertSecurityGroupRuleResponse(expected_secrule_copy, secrule)
        return secrule
