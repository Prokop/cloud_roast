...
class ScenarioMixin(object):
    ...
    def _create_server(cls, name, isolated_networks_to_connect=None,
                       public_and_service=False, public=True,
                       security_groups=None, scheduler_hints=None):
    ...
    @classmethod
    def _get_remote_client(cls, server, use_netw_name=False):
        if use_netw_name:
            netw = cls.net.behaviors.networks_behaviors.get_network(
                cls.public_network_id)
            netw_name = netw.response.entity.name
        else:
            netw_name = None
        ip_address = cls.compute.servers.behaviors.get_public_ip_address(
            server.entity, netw_name)
        remote_client = \
            cls.compute.servers.behaviors.get_remote_instance_client(
                server.entity,
                ip_address=ip_address, key=cls.keypair.private_key,
    ...
    @classmethod
    def _create_open_loginable_secgroup(cls):
