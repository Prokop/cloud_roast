...
class ServerSecurityGroupTest(ComputeFixture):
    ...
    def setUpClass(cls):
        ...
        name = rand_name("security_group")
        cls.security_group = cls.security_groups_client.create_security_group(
            name=name, description='test group').entity
        cls.server = cls.server_behaviors.create_active_server().entity
        cls.resources.add(cls.server.id, cls.servers_client.delete_server)
        cls.resources.add(cls.security_group.id, cls.security_groups_client.delete_security_group)
