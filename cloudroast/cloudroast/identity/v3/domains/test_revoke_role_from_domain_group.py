...
class TestRevokeRoleFromDomainGroup(IdentityV3Fixture):
    """Test Class for check token test case."""

    def setUp(self):
        super(TestRevokeRoleFromDomainGroup, self).setUp()
    ...
    def test_revoke_role_from_domain_group_using_domain_scoped_token(self):
        with self.mgen.create_metric("identity.revoke_role_from_domain_group_using_domain_scoped_token"):
    ...
    def test_revoke_role_from_domain_group_using_project_scoped_token(self):
        """ Expected to return 403 using project scoped token """
        with self.mgen.create_metric("identity.revoke_role_from_domain_group_using_project_scoped_token"):
            composite = self.project_composite
            response = composite.apis.roles.client.revoke_role_from_domain_group(
                domain_id=composite.user_config.scope_domain_id,
                group_id=composite.ident_config.group_for_test_roles,
