...
class TestListGroupsForUser(IdentityV3Fixture):

    def test_list_groups_for_user_using_domain_scoped_token(self):
        composite = self.domain_composite
        response = composite.apis.users.client.list_groups_for_user(
            user_id=composite.user_config.user_id)
        self.assertEqual(200, response.status_code)
        list_of_groups = response.entity
        self.assertIsNotNone(list_of_groups)

    def test_list_groups_for_user_using_project_scoped_token(self):
