...
class TestCheckRoleForProjectGroup(IdentityV3Fixture):
    """Test Class for check token test case."""

    def setUp(self):
        super(TestCheckRoleForProjectGroup, self).setUp()
    ...
    def tearDown(self):
        ...
        response = composite.apis.roles.client.revoke_role_from_project_group(
            ...
            role_id=composite.ident_config.role_for_grant_id)
        assert response.status_code == 204
        super(TestCheckRoleForProjectGroup, self).tearDown()

    def test_check_role_for_project_group_using_domain_scoped_token(self):
        composite = self.domain_composite
        response = composite.apis.roles.client.check_role_for_project_group(
            project_id=composite.user_config.project_id,
            group_id=composite.ident_config.group_for_test_roles,
            role_id=composite.ident_config.role_for_grant_id)
        self.assertEqual(204, response.status_code)

    def test_check_role_for_project_group_using_project_scoped_token(self):
        composite = self.project_composite
        response = composite.apis.roles.client.check_role_for_project_group(
            project_id=composite.user_config.project_id,
            group_id=composite.ident_config.group_for_test_roles,
