...
class TestRevokeRoleFromProjectGroup(IdentityV3Fixture):
    """Test Class for check token test case."""

    def setUp(self):
        super(TestRevokeRoleFromProjectGroup, self).setUp()
    ...
    def test_revoke_role_from_project_group_using_domain_scoped_token(self):
    ...
    def test_revoke_role_from_project_group_using_project_scoped_token(self):
        composite = self.project_composite
        response = composite.apis.roles.client.revoke_role_from_project_group(
            project_id=composite.user_config.project_id,
            group_id=composite.ident_config.group_for_test_roles,
